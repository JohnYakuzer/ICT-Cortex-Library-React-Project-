...
import './styles/librarian_profile.css';
    ...
    const handleSavePassword = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error('Greska pri resetovanju lozinke.');
            }
            ...
            if (result.status === 'success' || result.message?.toLowerCase().includes('uspjesno')) {
                ...
                showMessage(`Sifra za korisnika ${userData.first_name} uspjesno resetovana.`);
                closeResetModal();
            } else {
            ...
            showMessage('Greska, opet boze dragi: ' + error.message);
        }
    };

    const handleDeleteUser = async () => {
        if (!window.confirm(`Da li ste sigurni da zelite obrisati bibliotekara ${userData.first_name} ${userData.last_name}?`)) {
            return;
        }
        try {
            const token = localStorage.getItem('token');
            ...
            if (!response.ok) {
                throw new Error('Doslo je do greske pri brisanju bibliotekara');
            }

            const result = await response.json();
            showMessage(result.message || 'Bibliotekar je uspjesno obrisan!');
            navigate('/all_users');
        } catch (error) {
        ...
        return (
            <>
                ...
                <div className="fetching_user_content">
                    <h1>Ucitavanje podataka bibliotekara, molimo Vas pricekajte...</h1>
                </div>
            </>
        );
    }
        ...
        <>
            ...
            <div className="userprofile_canvas">
                <div className="userprofile_header">
                    ...
                    <div className="userprofile_path">
                        <p>
                            <Link to="/all_librarian" className="author_evid">Svi bibliotekari</Link> /{' '}
                            <Link to={`/user_profile/${userId}`} className="author_evid">{userData.id || 'user_id'}</Link>
                        </p>
                            ...
                            <div className="user_profile_drop_down_menu" onClick={() => setShowDropdown(!showDropdown)}>
                                ...
                                {showDropdown && (
                                    <div className="dropdown_menu">
                                        <p onClick={handleDeleteUser}>🗑 Izbrisi bibliotekara</p>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                <div className="user_profile_header_divider">
                        <hr />
                </div>
                    ...
                    <div className="reset_modal_content">
                        <div className="reset_modal_header">
                            <div className="reset_modal_reset_passwd_placeholder">
                                <h3>Resetuj sifru: {`${userData.first_name} ${userData.last_name}`}</h3>
                            </div>
